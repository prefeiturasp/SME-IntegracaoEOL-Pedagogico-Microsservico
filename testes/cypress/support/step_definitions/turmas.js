...
When('envio uma requisição POST para listar turmas regulares sem autenticação', () => {
  const apiUrl = getEnvOrFail('API_URL');
  const ue = getEnvOrFail('UE_CODIGO');
  const anoLetivo = getEnvOrFail('ANO_LETIVO');
  const modalidade = getEnvOrFail('MODALIDADE');
...
When('envio uma requisição GET para consultar dados de uma turma inexistente', () => {
  const apiUrl = getEnvOrFail('API_URL');
  const apiKey = getEnvOrFail('API_KEY');
...
Given('que possuo acesso à API de sincronizações institucionais de turma', () => {
  getEnvOrFail('API_URL');
  getEnvOrFail('API_KEY');
  getEnvOrFail('UE_CODIGO');
  getEnvOrFail('TURMA_CODIGO');
...
When('envio uma requisição GET para listar turmas históricas do professor', () => {
  const apiUrl = getEnvOrFail('API_URL');
  const apiKey = getEnvOrFail('API_KEY');
  const anoLetivo = getEnvOrFail('ANO_LETIVO');
